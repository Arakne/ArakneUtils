...
class BattlefieldSightTest {
    ...
    void between() {
        ...
        assertTrue(lineOfSight.between(map.get(126), map.get(123)));
        assertTrue(lineOfSight.between(map.get(123), map.get(126)));
    }
    ...
    private List<Integer> dumpLoS(@IndexFor("this.map") int cell) {
    ...
    @Test
    void getCellByCoordinates() {
        assertEquals(0, lineOfSight.getCellByCoordinates(0, 0).id());

        for (int id = 0; id < map.size(); ++id) {
            CoordinateCell<MyDofusCell> coords = new CoordinateCell<>(map.get(id));

            assertEquals(coords.cell(), lineOfSight.getCellByCoordinates(coords.x(), coords.y()));
        }
    }
    ...
    void from() {
        assertIterableEquals(
            new CellSight<>(lineOfSight, new CoordinateCell<>(map.get(123))).accessible(),
            lineOfSight.from(new CoordinateCell<>(map.get(123))).accessible()
        );
